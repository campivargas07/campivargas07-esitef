...
import { getEmailLogoDarkUrl, getEmailLogoUrl, getPublicSiteUrl } from "./site-url";
...
assert(
  getEmailLogoDarkUrl(base).endsWith("/img/Esitef_logo_icon_dark.png"),
  "dark logo path"
);
assert(getEmailLogoUrl(base).startsWith("https://"), "logo must be absolute https");
assert(
  getEmailLogoDarkUrl(base).startsWith("https://"),
  "dark logo must be absolute https"
);
