"use client";

import { FormEvent, useState } from "react";

const SOCIALS = [
  {
    label: "ESITEF",
    network: "Facebook",
    url: "https://www.facebook.com/esitef.sudamerica",
    icon: "facebook" as const,
  },
  {
    label: "ESITEF",
    network: "Instagram",
    url: "https://www.instagram.com/esitef_internacional/",
    icon: "instagram" as const,
  },
  {
    label: "Movement Therapy",
    network: "Facebook",
    url: "https://www.facebook.com/people/Movement-Therapy-by-Esitef/100066335085533/",
    icon: "facebook" as const,
  },
  {
    label: "Movement Therapy",
    network: "Instagram",
    url: "https://www.instagram.com/movementtherapy_esitef/",
    icon: "instagram" as const,
  },
];

function SocialIcon({ icon }: { icon: "facebook" | "instagram" }) {
  if (icon === "instagram") {
    return (
      <svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor" aria-hidden>
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
      </svg>
    );
  }

  return (
    <svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor" aria-hidden>
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  );
}

export function ContactoForm() {
  const [status, setStatus] = useState<"idle" | "loading" | "success" | "error">(
    "idle"
  );

  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    setStatus("loading");
    const form = new FormData(e.currentTarget);

    const res = await fetch("/api/contact", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        nombre: form.get("nombre"),
        email: form.get("email"),
        mensaje: form.get("mensaje"),
      }),
    });

    setStatus(res.ok ? "success" : "error");
    if (res.ok) e.currentTarget.reset();
  }

  return (
    <section className="contacto-section" aria-label="Contacto">
      <div className="contacto-inner">
        <div className="contacto-module esitef-module-shell">
          <div className="contacto-card esitef-module-card">
            <h1 className="contacto-title">Contacto</h1>

            {status === "success" ? (
              <div className="contacto-success" role="status">
                <p>Mensaje enviado. Te responderemos pronto.</p>
              </div>
            ) : (
              <>
                {status === "error" && (
                  <p className="contacto-error" role="alert">
                    Revisa los campos e inténtalo de nuevo.
                  </p>
                )}

                <form className="contacto-form" onSubmit={onSubmit} noValidate>
                  <div className="contacto-field">
                    <label htmlFor="contacto-nombre">
                      Nombre <span aria-hidden="true">*</span>
                    </label>
                    <input
                      type="text"
                      id="contacto-nombre"
                      name="nombre"
                      autoComplete="name"
                      required
                    />
                  </div>

                  <div className="contacto-field">
                    <label htmlFor="contacto-email">
                      Email <span aria-hidden="true">*</span>
                    </label>
                    <input
                      type="email"
                      id="contacto-email"
                      name="email"
                      autoComplete="email"
                      inputMode="email"
                      spellCheck={false}
                      required
                    />
                  </div>

                  <div className="contacto-field">
                    <label htmlFor="contacto-mensaje">
                      Mensaje <span aria-hidden="true">*</span>
                    </label>
                    <textarea
                      id="contacto-mensaje"
                      name="mensaje"
                      rows={4}
                      required
                    />
                  </div>

                  <button
                    type="submit"
                    className="contacto-btn"
                    disabled={status === "loading"}
                  >
                    {status === "loading" ? "Enviando…" : "Enviar"}
                  </button>
                </form>
              </>
            )}
          </div>
        </div>

        <div className="contacto-module esitef-module-shell">
          <div className="contacto-card esitef-module-card">
            <h2 className="contacto-title contacto-title--social">Redes sociales</h2>
            <ul className="contacto-social-list">
              {SOCIALS.map((item) => (
                <li key={`${item.label}-${item.network}`}>
                  <a
                    className="contacto-social-link"
                    href={item.url}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    <span className="contacto-social-icon" aria-hidden="true">
                      <SocialIcon icon={item.icon} />
                    </span>
                    <span className="contacto-social-text">
                      <span className="contacto-social-label">{item.label}</span>
                      <span className="contacto-social-network">{item.network}</span>
                    </span>
                  </a>
                </li>
              ))}
            </ul>
          </div>
        </div>
      </div>
    </section>
  );
}
